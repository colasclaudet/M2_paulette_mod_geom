...
void MainWindow::changeValuePointCut(OpenMesh::Vec3f first, OpenMesh::Vec3f last)
{
    ...
    ui->lb_firstX->setNum(ui->displayWidget->first_point_to_cut3D[0]);
    ui->lb_firstY->setNum(ui->displayWidget->first_point_to_cut3D[1]);
    ui->lb_firstZ->setNum(ui->displayWidget->first_point_to_cut3D[2]);

    ui->lb_lastX->setNum(ui->displayWidget->last_point_to_cut3D[0]);
    ui->lb_lastY->setNum(ui->displayWidget->last_point_to_cut3D[1]);
    ui->lb_lastZ->setNum(ui->displayWidget->last_point_to_cut3D[2]);
}
...
void MainWindow::on_pushButton_couper_clicked()
{
    ...
    // initialisation des couleurs et épaisseurs (sommets et arêtes) du mesh
    //resetAllColorsAndThickness(&frag1);
    MyMesh::VertexHandle v1 = frag1.add_vertex(P1);
    MyMesh::VertexHandle v2 =frag1.add_vertex(P2);
    MyMesh::VertexHandle v3 =frag1.add_vertex(P3);

    frag1.set_color(v1,MyMesh::Color(0,255,0));
    frag1.data(v1).thickness = 10;

    frag1.set_color(v2,MyMesh::Color(0,255,0));
    frag1.data(v2).thickness = 10;

    frag1.set_color(v3,MyMesh::Color(0,255,0));
    frag1.data(v3).thickness = 10;
    // on affiche le maillage
    QVector<MyMesh * > listmeshes;
    listmeshes.push_back(&frag1);
    listmeshes.push_back(&frag2);
    displayMeshes(listmeshes);
    this->changeValuePointCut(ui->displayWidget->first_point_to_cut3D,ui->displayWidget->last_point_to_cut3D);
    this->list_meshes_to_save.clear();
    this->list_meshes_to_save.push_back(frag1);
    this->list_meshes_to_save.push_back(frag2);
    //displayMesh(&frag1);
    //displayMesh(&cpy);
}
